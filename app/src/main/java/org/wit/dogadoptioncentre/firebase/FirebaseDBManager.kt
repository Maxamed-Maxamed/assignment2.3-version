...
object FirebaseDBManager : AdoptionStore {
    ...
    override fun findById(
        userid: String,
        adoptionId: String,
        adoption: MutableLiveData<AdoptionModel>
    ) {

        database.child("user-adoptions").child(userid)
            .child(adoptionId).get().addOnSuccessListener {
                adoption.value = it.getValue(AdoptionModel::class.java)
    ...
    override fun update(userid: String, adoptionid: String, adoptions: AdoptionModel) {

        val adoptionValues = adoptions.toMap()

        val childUpdate : MutableMap<String, Any?> = HashMap()
        childUpdate["adoptions/$adoptionid"] = adoptionValues
        childUpdate["user-adoptions/$userid/$adoptionid"] = adoptionValues
